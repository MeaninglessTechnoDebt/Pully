...
contract NFTAuctionBase is Pausable {
	...
	function _escrow(address, uint _partId) internal {
		...
		nftContract.transferFrom(msg.sender, this, _partId);
	}
	...
	function _transfer(address _purchasor, uint256 _partId) internal {
		...
		// actual transfer
		nftContract.transferFrom(this, _purchasor, _partId);
	}
...
/**
 * @title DutchAuction 
 * @dev Unfortunately DutchX does not support ERC721 ((((( 
 * So we built our own simple Dutch auction
 */
contract DutchAuction is NFTAuctionBase {
	...
	function createAuction( uint256 _partId, uint256 _startPrice, uint256 _endPrice, uint256 _duration, address _seller ) external whenNotPaused {
		...
		require(nftContract.ownerOf(_partId) == msg.sender);
